...
void second(CSV fp, char* fn){
	...
	if (op2 == 1){
		if (/*GB_UpdateDataBase(fp, fn, l)*/) puts("Database updated successfully. Returning to the main menu.");
		else {
...
void browse(){
	...
	switch(op){
		...
		case 3:
			fgets(tag, 51, stdin);
			target = search_keyword(tag, root, l->size);
			printsite(target);
			break;
		default: return;
	}
